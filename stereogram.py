...
def apply_divergence_row(color_img, depth_img, divergence_px, separation_px, row, derived_img):
  EPSILON = 1e-7
...
class WarpImage(BaseInvocation):
  ...
  def apply_divergence(self, color_img, depth_img, divergence_px, separation_px):
    ...
    for row in range(h):
      apply_divergence_row(color_img, depth_img, divergence_px, separation_px, row, derived_img)
    return derived_img
